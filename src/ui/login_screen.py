...
class LoginScreen:
    ...
    def attempt_login(self):
        ...
        if landlord_service.get_user(entered_username) is None:
            messagebox.showinfo(title='Invalid input', message='Username not found, please check your input!', icon='error')
            self._stay_on_screen()
        else:
            password_in_database = landlord_service.get_password(entered_username)
            if password_in_database == str(entered_password):
                ...
                messagebox.showinfo(title='Invalid input', message='Incorrect password, please check your input!', icon='error')
                self._stay_on_screen()
    ...
    def initialize(self):
        ...
        login_button = Button(master=self._frame, text='Login', command=self.attempt_login)

        create_account_button = Button(master=self._frame, text='Sign up', command=self._go_to_signup_screen)
