...
class InsertUnitsScreen:
    ...
    def create_unit_to_database(self):
        ...
        if self._input_not_empty_check(chosen_address, label='Chosen address field') == False:
            ...
            messagebox.showinfo(
                title='Success', message=f'Unit succesfully created')
            self._main_menu()
    ...
    def initialize(self):
        ...
        construction_year_label = Label(
            master=self._frame, text='Construction year')
        self._construction_year_field_input = Entry(master=self._frame)
        sewage_year_label = Label(
            master=self._frame, text='Sewage repair year (optional)')
        self._sewage_year_field_input = Entry(master=self._frame)
        facade_year_label = Label(
            master=self._frame, text='Facade repair year (optional)')
        self._facade_year_field_input = Entry(master=self._frame)
        windows_year_label = Label(
            master=self._frame, text='Window repair year (optional)')
        self._windows_year_field_input = Entry(master=self._frame)
        elevator_year_label = Label(
            master=self._frame, text='Elevator repair year (optional)')
        self._elevator_year_field_input = Entry(master=self._frame)
        elevator_button = Checkbutton(
            master=self._frame, text='Building has elevator (optional)')
        ...
        asking_price_label = Label(
            master=self._frame, text='Asking price (optional)')
