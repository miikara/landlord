...
class InsertRentsScreen:
    ...
    def create_rent_to_database(self):
        ...
        if self._input_not_empty_check(chosen_unit_id, label='Chosen unit id') == False:
            ...
            self._stay_on_screen()
        elif self._end_previous_rent_field_state.instate(['selected']):
            rent_id_to_end = landlord_service.get_latest_rent_id(
                chosen_unit_id)
            ...
            messagebox.showinfo(
                title='Success', message=f'Rent succesfully created')
            self._main_menu()
        else:
            landlord_service.create_rent(
                chosen_unit_id, chosen_start_date, chosen_amount, chosen_due_dom)
    ...
    def initialize(self):
        ...
        start_date_label = tk.Label(master=self._frame, text='Rent start date')
