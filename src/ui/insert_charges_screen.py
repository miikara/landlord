...
class InsertChargesScreen:
    ...
    def create_maintenance_charge_to_database(self):
        ...
        if self._end_previous_charge_field_state.instate(['selected']):
            charge_id_to_end = landlord_service.get_latest_maintenance_charge_id(chosen_unit_id)
            landlord_service.end_maintenance_charge(charge_id_to_end, chosen_start_date)
    ...
    def initialize(self):
        ...
        start_date_label = Label(master=self._frame, text='Charge start date')
        ...
        due_dom_label = Label(master=self._frame, text='Charge due day of month')
        self._due_dom_field_input = Entry(master=self._frame) 
        end_previous_label = Checkbutton(master=self._frame, text='End previous charge') 
        self._end_previous_charge_field_state = end_previous_label

        # Input field grids here NOT DONE
