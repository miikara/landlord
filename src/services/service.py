...
class LandlordService:
    ...
    def create_unit(self, chosen_address, chosen_location, chosen_construction_year, chosen_sewage_year, chosen_facade_year, chosen_windows_year, chosen_elevator_year, chosen_has_elevator, chosen_square_meters, chosen_floor, chosen_asking_price, chosen_purchase_price, chosen_acquired_date):
        """Creates a unit to database. Sold date and owned can not be chosen"""
        username_used = str(self._user)
        unit_to_create = Unit(username=username_used, address=chosen_address, location=chosen_location, construction_year=chosen_construction_year, sewage_year=chosen_sewage_year, facade_year=chosen_facade_year, windows_year=chosen_windows_year, elevator_year=chosen_elevator_year,
                              has_elevator=chosen_has_elevator, square_meters=chosen_square_meters, floor=chosen_floor, asking_price=chosen_asking_price, purchase_price=chosen_purchase_price, acquired_date=chosen_acquired_date, sold_date='2098-01-01', owned=1)
    ...
    def sell_unit_and_set_sold_date(self, chosen_unit_id, chosen_sold_date):
        ...
        if int(chosen_unit_id) in list_of_units:
            ...
            self._unit_repository.set_sold_date(
                chosen_sold_date, chosen_unit_id)
    ...
    def create_lease(self, chosen_unit_id, chosen_start_date, chosen_end_date_on_contract, chosen_tenant, chosen_original_monthly_rent, chosen_maximum_annual_rent_increase, chosen_rent_due_date, chosen_deposit):
        lease_to_create = Lease(chosen_unit_id, chosen_start_date, chosen_end_date_on_contract, chosen_tenant,
    ...
    def get_latest_active_lease_tenant(self, chosen_unit_id):
        latest_active_lease = self._lease_repository.get_latest_start_date_active_lease(
            chosen_unit_id)
    ...
    def create_maintenance_charge(self, chosen_unit_id, chosen_start_date, chosen_amount, chosen_due_dom):
        ...
        maintenance_charge_to_create = Charge(
            unit_id=chosen_unit_id, start_date=chosen_start_date, amount=chosen_amount, due_dom=chosen_due_dom)
        self._charge_repository.add_charge_to_database(
            maintenance_charge_to_create)
        return maintenance_charge_to_create

    def get_latest_maintenance_charge_id(self, chosen_unit_id):
        result_charge_id = self._charge_repository.get_unit_ids_latest_maintenance_charge_id(
            chosen_unit_id)
        return result_charge_id

    def end_maintenance_charge(self, chosen_charge_id, chosen_date):
        self._charge_repository.set_charge_id_end_date(
            chosen_charge_id, chosen_date)
    ...
    def create_rent(self, chosen_unit_id, chosen_start_date, chosen_amount, chosen_due_dom):
        ...
        chosen_lease_id = self._lease_repository.get_latest_created_active_lease_id(
            chosen_unit_id)
        rent_to_create = Rent(lease_id=chosen_lease_id, start_date=chosen_start_date,
                              amount=chosen_amount, due_dom=chosen_due_dom)
        self._rent_repository.add_rent_to_database(rent_to_create)
        return rent_to_create

    def get_latest_rent_id(self, chosen_unit_id):
        result_rent_id = self._rent_repository.get_unit_ids_latest_rent_id(
            chosen_unit_id)
    ...
    def get_noi(self, chosen_unit_id, chosen_months=12):
        annual_rent = self._rent_repository.get_unit_ids_latest_rent_amount(
            chosen_unit_id) * chosen_months
        annual_charges = self._charge_repository.get_unit_ids_latest_maintenance_charge_amount(
            chosen_unit_id) * 12
    ...
    def get_cap_rate(self, chosen_unit_id, chosen_months=12, chosen_purchase_tax_rate=2):
        ...
        annual_charges = self._charge_repository.get_unit_ids_latest_maintenance_charge_amount(
            chosen_unit_id) * 12
        purchase_price = self._unit_repository.get_unit_ids_purchase_price(
            chosen_unit_id) * (1+chosen_purchase_tax_rate/100)
        cap_rate = (annual_rent - annual_charges) / purchase_price
        return cap_rate

    def get_units_history(self, chosen_username):
        dates, units = self._unit_repository.get_unit_count_time_series(
            chosen_username)
        return dates, units

    def get_lease_history(self, chosen_username):
        dates, occupants = self._lease_repository.get_lease_count_time_series(
            chosen_username)
    ...
    def get_occupancy_history(self, chosen_username):
        dates, units = self._unit_repository.get_unit_count_time_series(
            chosen_username)
        occupancy_dates, occupants = self._lease_repository.get_lease_count_time_series(
            chosen_username)
