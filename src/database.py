...
def create_units_table(connection):
    with connection:
        connection.execute(
            ...
            square_meters NUM, floor INTEGER, asking_price NUM, purchase_price NUM, acquired_date TIMESTAMP, sold_date TIMESTAMP, owned INTEGER, \
...
def create_leases_table(connection):
    with connection:
        connection.execute(
            ...
            (lease_id INTEGER PRIMARY KEY, unit_id INTEGER, created_time TIMESTAMP, start_date TIMESTAMP, end_date TIMESTAMP, end_date_on_contract TIMESTAMP, tenant TEXT, contract_rent NUM, maximum_annual_rent_increase NUM, rent_due_date INTEGER, deposit NUM, FOREIGN KEY (unit_id) REFERENCES units (unit_id));')
...
def create_rents_table(connection):
    with connection:
        connection.execute(
            ...
            (rent_id INTEGER PRIMARY KEY, lease_id INTEGER, start_date TIMESTAMP, amount NUM, due_dom INTEGER, end_date TIMESTAMP, \
