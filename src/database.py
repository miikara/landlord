...
def create_units_table(connection):
    with connection:
        connection.execute(
            'CREATE TABLE IF NOT EXISTS units (unit_id INTEGER PRIMARY KEY, username TEXT, address TEXT, location TEXT, square_meters NUM, asking_price NUM, purchase_price NUM, unit_date TIMESTAMP, owned INTEGER);')
